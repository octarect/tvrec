...
const videoCodec = process.argv[2];
const size = process.argv[3];
const aspectRatio = process.argv[4];
const videoBitRate = process.argv[5];
...
// ビデオストリーム設定
Array.prototype.push.apply(args, ['-map', '0:v', '-c:v', videoCodec]);
// オーディオストリーム設定
if (isDualMono) {
    ...
    Array.prototype.push.apply(args, ['-map', '0:a:0']);
}
Array.prototype.push.apply(args, ['-c:a', 'copy', '-bsf:a', 'aac_adtstoasc', '-ac', '2']);
// 品質設定
Array.prototype.push.apply(args, ['-s', size, '-aspect', aspectRatio, '-b:v', videoBitRate]);
// 出力ファイル
